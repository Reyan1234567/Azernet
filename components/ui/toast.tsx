...
export function Toast({
  ...
  // Calculate expanded width with safe area consideration
  const EXPANDED_WIDTH = screenWidth - Math.max(insets.left, 16) - Math.max(insets.right, 16);
  ...
  const getTopPosition = () => {
    // Use safe area top inset for better positioning
    const topOffset = Math.max(insets.top, Platform.OS === 'ios' ? 59 : 20);
    return topOffset + index * (EXPANDED_HEIGHT + TOAST_MARGIN);
  };
